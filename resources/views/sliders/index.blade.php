    ...
    <div class="container my-5">
        <div class="card shadow-lg rounded-lg border-0">
            <div
                ...
                <button type="button" class="btn btn-light btn-sm shadow-sm px-4" data-toggle="modal"
                    data-target="#addSliderModal">
                    <i class="fa fa-plus mr-1"></i> إضافة صورة جديدة
                </button>

                {{-- مودال إضافة صورة --}}
                <div class="modal fade" id="addSliderModal" tabindex="-1" aria-labelledby="addSliderLabel"
                    aria-hidden="true">
                        ...
                        <tbody>
                            @forelse($sliders as $slider)
                                <tr class="bg-white shadow-sm rounded-lg">
                                    ...
                                    <td>
                                        @if ($slider->image)
                                            <img src="{{ asset('storage/' . $slider->image) }}" alt="Banner Image"
                                    ...
                                    <td>
                                        {{-- زر التعديل --}}
                                        <button type="button" class="btn btn-outline-primary btn-sm mr-2 mb-1"
                                            data-toggle="modal" data-target="#editSliderModal{{ $slider->id }}">
                                            <i class="fa fa-edit"></i> تعديل
                                        </button>

                                        {{-- مودال التعديل --}}
                                        <div class="modal fade" id="editSliderModal{{ $slider->id }}" tabindex="-1"
                                            aria-labelledby="editSliderLabel{{ $slider->id }}" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content rounded-lg shadow">
                                                    <form action="{{ route('sliders.update', $slider->id) }}" method="POST"
                                                        ...
                                                        @method('PUT')
                                                        <div class="modal-header border-0">
                                                            <h5 class="modal-title" id="editSliderLabel{{ $slider->id }}">
                                                                تعديل صورة البانر</h5>
                                                        ...
                                                        <div class="modal-body text-dark">
                                                            <div class="form-group">
                                                                <label for="image-{{ $slider->id }}">اختر صورة جديدة
                                                                    (اتركها فارغة للاحتفاظ بالصورة الحالية)</label>
                                                            ...
                                                            <div class="text-center mt-3">
                                                                <img src="{{ asset('storage/' . $slider->cover) }}"
                                        ...
                                        {{-- زر الحذف --}}
                                        <x-model name="delete-{{ $slider->id }}" status="danger" icon="fa fa-trash"
                                            title="حذف" message="هل أنت متأكد من حذف هذه الصورة؟">
