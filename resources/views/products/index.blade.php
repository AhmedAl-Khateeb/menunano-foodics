...
@error('selling_price') <div class="invalid-feedback">{{ $message }}</div> @enderror
    ...
    <div id="sizes-wrapper">
        <div class="row size-item mb-2">
            ...
            <div class="col-md-3">
                <input type="number" name="sizes[0][Purchase_price]" class="form-control" placeholder="سعر الشراء" required>
            </div>



            <div class="col-md-3">
                <input type="number" name="sizes[0][selling_price]" class="form-control" placeholder="سعر البيع" required>
            </div>
...
{{ $product->selling_price !== null ? number_format($product->selling_price, 2) . '' : '—' }}
        ...
        <td>
            @if($product->cover)
                <img src="{{ asset('Attachfile/products/'.$product->cover) }}" alt="Cover Image" class="img-thumbnail rounded" style="width: 80px; height: 80px; object-fit: cover;">
            @else
...
<div class="col-md-6 mb-3">
<label for="price-{{ $product->id }}">سعر الشراء</label>
<input type="number" name="Purchase_price-{{ $product->id }}" id="Purchase_price"
class="form-control @error('Purchase_price') is-invalid @enderror"
step="0.01" min="0" value="{{ old('Purchase_price', $product->Purchase_price) }}">
@error('price') <div class="invalid-feedback">{{ $message }}</div> @enderror
</div>


<div class="col-md-6 mb-3">
<label for="price-{{ $product->id }}">سعر البيع</label>
<input type="number" name="selling_price-{{ $product->id }}" id="selling_price"
class="form-control @error('selling_price') is-invalid @enderror"
step="0.01" min="0" value="{{ old('selling_price', $product->selling_price) }}">
@error('selling_price') <div class="invalid-feedback">{{ $message }}</div> @enderror
</div>
...
ِ                                                <div class="col-md-6 mb-3">
                                        <label for="cover-{{ $product->id }}">الصورة (اتركها فارغة للاحتفاظ بالصورة الحالية)</label>
                                        <input type="file" name="cover" id="cover-{{ $product->id }}" class="form-control @error('cover') is-invalid @enderror" accept="image/*" required>
                                        @error('cover') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                    </div>
                                </div>
                                ...
                                <div id="sizes-wrapper-{{ $product->id }}">
                                    @foreach ($product->sizes as $i => $size)
                                        <div class="row size-item mb-2">
                                            ...
                                            <div class="col-md-3">
                                                <input type="number" name="sizes[{{ $i }}][Purchase_price]" class="form-control" value="{{ $size->Purchase_price }}" placeholder="سعر الشراء" required>
                                            </div>

                                            <div class="col-md-3">
                                                <input type="number" name="sizes[{{ $i }}][selling_price]" class="form-control" value="{{ $size->selling_price }}" placeholder="سعر البيع" required>
                                            </div>
a
