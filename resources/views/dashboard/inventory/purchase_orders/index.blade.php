        ...
        <div class="card">
            <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-2">
                ...
                <div class="d-flex flex-wrap align-items-center gap-2">
                    <form action="{{ route('inventory.purchase-orders.index') }}" method="GET" class="d-flex flex-wrap gap-2">
                        ...
                        <select name="status" class="form-control form-control-sm" style="width:160px;">
                            ...
                            <option value="partial_received" {{ request('status') == 'partial_received' ? 'selected' : '' }}>استلام جزئي</option>
                        ...
                        <tbody>
                            @forelse($purchaseOrders as $order)
                                <tr>
                                    ...
                                    <td>{{ number_format($order->total ?? 0, 3) }}</td>
                                    <td>
                                        @php
                                            $statusMap = [
                                            ...
                                            $statusData = $statusMap[$order->status] ?? ['secondary', $order->status];
                                        @endphp
                                    ...
                                    <td>
                                        <div class="d-flex gap-1 justify-content-center flex-wrap">
                                            ...
                                            <a href="{{ route('inventory.purchase-orders.edit', $order->id) }}" class="btn btn-info btn-sm">
                                                <i class="fas fa-edit"></i>
                                            </a>

                                            <form action="{{ route('inventory.purchase-orders.destroy', $order->id) }}" method="POST" class="d-inline delete-form">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" class="btn btn-danger btn-sm delete-btn" data-name="{{ $order->po_number }}">
                        ...
                        <div class="card mb-3 border shadow-none rounded-lg">
                            <div class="card-body p-3">
                                ...
                                <div class="d-flex gap-2 border-top pt-2 flex-wrap">
                                    @if($order->status === 'draft')
                                        <form action="{{ route('inventory.purchase-orders.approve', $order->id) }}" method="POST" class="flex-grow-1">
                                            @csrf
                                            <button type="submit" class="btn btn-success btn-sm w-100">اعتماد</button>
                                        </form>
                                    @endif

                                    <a href="{{ route('inventory.purchase-orders.edit', $order->id) }}" class="btn btn-info btn-sm flex-grow-1">
                                        تعديل
                                    </a>

                                    <form action="{{ route('inventory.purchase-orders.destroy', $order->id) }}" method="POST" class="flex-grow-1 delete-form">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" class="btn btn-danger btn-sm w-100 delete-btn" data-name="{{ $order->po_number }}">
                                            حذف
                                        </button>
                                    </form>
                                </div>
