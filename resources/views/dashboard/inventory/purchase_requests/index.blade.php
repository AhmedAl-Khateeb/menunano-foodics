            ...
            <div class="card">
                <div class="card-header d-flex flex-wrap justify-content-between align-items-center">
                    ...
                    <div class="d-flex flex-wrap gap-2">
                        <form action="{{ route('inventory.purchase-requests.index') }}" method="GET"
                ...
                <div class="card-body p-0">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                                ...
                                                $statusData = $statusMap[$requestItem->status] ?? [
                                                    'secondary',
                                                    $requestItem->status,
                                                ];
                                            @endphp
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center flex-wrap" style="gap: 5px;">
                                                @if ($requestItem->status === 'draft')
                                                    <form
                                                        action="{{ route('inventory.purchase-requests.approve', $requestItem->id) }}"
                                                ...
                                                <button type="button" class="btn btn-info btn-sm" data-toggle="modal"
                                                    data-target="#itemsModal{{ $requestItem->id }}" title="عرض الأصناف">
                    ...
                    <div class="d-block d-md-none p-3">
                        @forelse($purchaseRequests as $requestItem)
                            <div class="card mb-3 shadow-none border">
                                <div class="card-body">
                                    ...
                                    <div class="small text-muted mb-2">
                                        <div>تاريخ الطلب:
                                            {{ \Carbon\Carbon::parse($requestItem->request_date)->format('Y-m-d') }}</div>
                                    ...
                                    <div class="d-flex flex-wrap" style="gap: 6px;">
                                        ...
                                        <button type="button" class="btn btn-info btn-sm flex-fill" data-toggle="modal"
                            ...
                            <!-- Mobile Modal -->
                            <div class="modal fade" id="itemsModalMobile{{ $requestItem->id }}" tabindex="-1"
                                        ...
                                        <div class="modal-body">
                                            @forelse($requestItem->items as $item)
                                                <div class="border rounded p-2 mb-2">
                                                    <div><strong>الصنف:</strong> {{ $item->rawMaterial->name ?? '-' }}
                                                    </div>
                                                    <div><strong>الوحدة:</strong> {{ $item->unit->name ?? '-' }}</div>
                                                    <div><strong>المطلوب:</strong>
                                                        {{ number_format($item->requested_quantity, 3) }}</div>
                                                    <div><strong>المعتمد:</strong>
                                                        {{ number_format($item->approved_quantity, 3) }}</div>
