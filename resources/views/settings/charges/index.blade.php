    ...
    <div x-data="{
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            @if ($charges->isEmpty())
                <div class="bg-white rounded-3xl p-16 text-center border border-dashed border-gray-300">
                    ...
                    <p class="text-gray-500 max-w-md mx-auto mb-8">قم بتعريف الضرائب والرسوم (مثل القيمة المضافة 15٪)
                        لتطبيقها على المنتجات والطلبات.</p>
                    <button @click="openAddModal()"
                        ...
                        <div
                            ...
                            <div class="relative">
                                ...
                                <div class="flex items-baseline gap-1 mb-4">
                                    <span
                                        class="text-3xl font-black text-gray-900 tracking-tight">{{ floatval($charge->value) }}</span>
                                    <span
                                        class="text-sm font-bold text-gray-500">{{ $charge->type == 'percentage' ? '%' : 'SAR' }}</span>
                                </div>

                                <div class="flex items-center justify-between pt-4 border-t border-gray-50">
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="w-2 h-2 rounded-full {{ $charge->is_active ? 'bg-green-500' : 'bg-gray-300' }}"></span>
                                        <span
                                            class="text-xs font-bold {{ $charge->is_active ? 'text-green-600' : 'text-gray-400' }}">
                                            {{ $charge->is_active ? 'نشط' : 'غير نشط' }}
                                        </span>
                                    </div>
                                    @if ($charge->classification === 'tax')
                                        <span
                                            class="px-2 py-0.5 rounded text-[10px] font-bold {{ $charge->is_inclusive ? 'bg-indigo-100 text-indigo-700' : 'bg-orange-100 text-orange-700' }}">
                                ...
                                <div class="mt-2 flex flex-wrap gap-1">
                                    @if (is_array($charge->applicable_order_types))
                                        @if (in_array('dining_in', $charge->applicable_order_types))
                                            <span
                                                class="px-2 py-0.5 rounded text-[10px] font-bold bg-blue-100 text-blue-700">ديكور</span>
                                        @endif
                                        @if (in_array('takeaway', $charge->applicable_order_types))
                                            <span
                                                class="px-2 py-0.5 rounded text-[10px] font-bold bg-orange-100 text-orange-700">سفري</span>
                                        @endif
                                        @if (in_array('delivery', $charge->applicable_order_types))
                                            <span
                                                class="px-2 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-700">توصيل</span>
                                        @endif
                                    @endif
                                </div>
                                @if ($charge->description)
                                    <div class="group/tooltip relative">
                                        ...
                                        <div
                                            class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1 bg-gray-900 text-white text-xs rounded-lg opacity-0 group-hover/tooltip:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-20">
        ...
        <div x-show="showAddModal || showEditModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;"
            ...
            <div class="fixed inset-0 bg-gray-900/50 backdrop-blur-sm transition-opacity"
                @click="showAddModal = false; showEditModal = false"></div>
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-xl font-bold text-gray-900"
                                x-text="showEditModal ? 'تعديل البيانات' : 'إضافة ضريبة / رسم جديد'"></h3>
                        ...
                        <form
                            :action="showEditModal ? '{{ url('settings/charges') }}/' + form.id :
                                '{{ route('charges.store') }}'"
                            method="POST" id="chargeForm">
                            @csrf
                            <input type="hidden" name="_method" :value="form._method">

                            <!-- Name -->
                            <div class="mb-5">
                                <label class="block text-sm font-bold text-gray-700 mb-2">المسمى <span
                            ...
                            <div class="mb-5 bg-gray-50 p-3 rounded-xl border border-gray-100 flex gap-4">
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="classification" value="tax"
                                        x-model="form.classification" class="text-indigo-600 focus:ring-indigo-500">
                                    <span class="text-sm font-bold text-gray-700">ضريبة (Tax)</span>
                                </label>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="radio" name="classification" value="fee"
                                        x-model="form.classification" class="text-indigo-600 focus:ring-indigo-500">
                            ...
                            <div class="grid grid-cols-2 gap-4 mb-5">
                                <div>
                                    ...
                                    <select name="type" x-model="form.type" required
                                        class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 py-2.5 bg-white">
                                        <option value="percentage">نسبة مئوية (%)</option>
                                        <option value="fixed">مبلغ ثابت (SAR)</option>
                                    </select>
                                </div>
                                <div>
                                    <label class="block text-sm font-bold text-gray-700 mb-2">القيمة <span
                                            class="text-red-500">*</span></label>
                                    <div class="relative">
                                        <input type="number" name="value" x-model="form.value" step="0.01"
                                            min="0" required
                                        ...
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                            <span class="text-gray-500 font-bold sm:text-sm"
                                                x-text="form.type == 'percentage' ? '%' : 'SAR'"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-5" x-show="form.classification === 'tax'" x-transition>
                                <label
                                    class="flex items-center justify-between cursor-pointer group bg-white border border-gray-200 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                                    ...
                                    <div class="relative">
                                        <input type="checkbox" name="is_inclusive" class="sr-only peer"
                                            x-model="form.is_inclusive">
                                        <div
                            ...
                            <div class="mb-5 bg-gray-50 p-3 rounded-xl border border-gray-100">
                                ...
                                <div class="flex flex-col gap-2">
                                    <label class="flex items-center gap-2 cursor-pointer p-2 rounded hover:bg-gray-100">
                                        <input type="checkbox" value="dining_in" x-model="form.applicable_order_types"
                                            class="text-indigo-600 focus:ring-indigo-500 rounded">
                                        <span class="text-sm text-gray-700 font-bold">داخلي (Dining In)</span>
                                    </label>
                                    <label class="flex items-center gap-2 cursor-pointer p-2 rounded hover:bg-gray-100">
                                        <input type="checkbox" value="takeaway" x-model="form.applicable_order_types"
                                            class="text-indigo-600 focus:ring-indigo-500 rounded">
                                        <span class="text-sm text-gray-700 font-bold">سفري (Takeaway)</span>
                                    </label>
                                    <label class="flex items-center gap-2 cursor-pointer p-2 rounded hover:bg-gray-100">
                                        <input type="checkbox" value="delivery" x-model="form.applicable_order_types"
                                            class="text-indigo-600 focus:ring-indigo-500 rounded">
                                ...
                                <label class="flex items-center justify-between cursor-pointer group">
                                    <span class="flex flex-col">
                                        ...
                                        <span class="text-xs text-gray-500 mt-0.5"
                                            x-text="form.is_active ? 'نشط ويمكن استخدامه' : 'معطل مؤقتاً'"></span>
                                    </span>
                                    <div class="relative">
                            ...
                            <div class="flex items-center gap-3 mt-8">
                                <button type="button" @click="showAddModal = false; showEditModal = false"
                                    class="flex-1 bg-white text-gray-700 px-4 py-3 rounded-xl border border-gray-300 font-bold hover:bg-gray-50 transition-colors">
                                    إلغاء
                                </button>
                                <button type="submit"
                                    class="flex-[2] bg-indigo-600 text-white px-4 py-3 rounded-xl font-bold hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200">
        ...
        <div x-show="showDeleteModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;"
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div class="relative transform overflow-hidden rounded-2xl bg-white text-right shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md"
                    ...
                    <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                        <div
                            class="mx-auto flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-full bg-red-50 sm:mx-0 sm:h-16 sm:w-16 mb-4">
                        ...
                        <div class="text-center sm:text-right">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500">
                                    هل أنت متأكد من حذف <span class="font-bold text-gray-900"
                                        x-text="editingCharge?.name"></span>؟
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 gap-2">
                        <form :action="'{{ url('settings/charges') }}/' + editingCharge?.id" method="POST"
                            class="w-full sm:w-auto">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="inline-flex w-full justify-center rounded-xl bg-red-600 px-6 py-3 text-sm font-bold text-white shadow-sm hover:bg-red-500 sm:w-auto transition-colors">
                                حذف نهائي
                            </button>
                        </form>
                        <button type="button" @click="showDeleteModal = false"
                            class="mt-3 inline-flex w-full justify-center rounded-xl bg-white px-6 py-3 text-sm font-bold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto transition-colors">
                            إلغاء
                        </button>
                    </div>
                </div>
