                    ...
                    <tbody>
                        @forelse($categories as $category)
                            <tr class="bg-white shadow-sm rounded-lg">
                                ...
                                <td>
                                    @if($category->cover)
                                        <img src="{{ asset('storage/app/public/'.$category->cover) }}" alt="Cover Image" class="img-thumbnail rounded" style="width: 80px; height: 80px; object-fit: cover;">
                                    @else
                                ...
                                <td>
                                    {{-- زر التعديل --}}
                                    <button type="button" class="btn btn-outline-success btn-sm mr-2 mb-1" data-toggle="modal" data-target="#edit{{ $category->id }}">
                                        <i class="fa fa-edit"></i> تعديل
                                    </button>

                                    {{-- مودال التعديل --}}
                                    <div class="modal fade" id="edit{{ $category->id }}" tabindex="-1" aria-labelledby="editLabel{{ $category->id }}" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content rounded-lg shadow">
                                                <form action="{{ route('categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" novalidate>
                                                    ...
                                                    @method('PUT')
                                                    <div class="modal-header border-0">
                                                        <h5 class="modal-title" id="editLabel{{ $category->id }}">تعديل الفئة</h5>
                                                        <button type="button" class="close" data-dismiss="modal" aria-label="إغلاق">
                                                            <span aria-hidden="true" style="font-size: 1.5rem;">&times;</span>
                                                        </button>
                                                    </div>
                                                    <div class="modal-body text-dark">
                                                        <div class="form-group">
                                                            ...
                                                            <input type="text" name="name" id="name-{{ $category->id }}" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $category->name) }}" required autofocus>
                                                            @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="cover-{{ $category->id }}">الصورة (اتركها فارغة للاحتفاظ بالصورة الحالية)</label>
                                                            <input type="file" name="cover" id="cover-{{ $category->id }}" class="form-control @error('cover') is-invalid @enderror" accept="image/*">
                                                            @error('cover') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                                        </div>
                                                    </div>
                                                    <div class="modal-footer border-0 justify-content-between">
                                    ...
                                    {{-- زر الحذف --}}
                                    <x-model name="delete-{{ $category->id }}" status="danger" icon="fa fa-trash" title="حذف" message="هل أنت متأكد من حذف {{ $category->name }}؟">
